...
export function initPayment() {
  ...
  if (window.config.host !== 'pay.seerbitapi.com') {
    window.load()
  }

  // Define our constructor
  window.SeerbitPay = function () {
    ...
    if (arguments[0] && typeof arguments[0] === 'object') {
      ...
      _.options.payLink = !(_.options.email && _.options.amount)
    ...
    function buildOut() {
      ...
      if (!(_.options.email && _.options.amount)) {
      ...
      style.innerHTML = `.lds-ring {
        ...
        .lds-ring div {
          ...
          height: 31px;
          margin: 6px;
          border: 2px solid ${_.options.payLink ? '#000000' : '#ffffff'};
          border-radius: 50%;
          animation: lds-ring 0.5s cubic-bezier(0.5, 0, 0.5, 1) infinite;
          border-color: ${
            _.options.payLink ? '#000000' : '#ffffff'
    ...
    postMessage
    function isClose() {
      // document.querySelector("iframe#seerbit-frame").src = window.config.url;
      document.querySelector('iframe#seerbit-frame').style.display = 'none'
      document.querySelector('iframe#seerbit-framex').style.display = 'none'
      // document.querySelector("iframe#seerbit-frame").src = window.config.url;
      // document.querySelector('iframe#seerbit-frame').remove()
    ...
    function seerbitGetCallback(message) {
      ...
      if (
        (_.callback && message && message.data && message.data.code === '00') ||
